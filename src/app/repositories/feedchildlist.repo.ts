...
`
  ...
  async getAllFeedListItemByCategory(category: string) {
    const sql = `SELECT category_id,category_name, company_name, company_site,company_feed_url, company_logo_url, company_description, feed_title, feed_content, feed_image,  feed_author, feed_article_url, date_posted  FROM feed_child_list WHERE category_name='${category}' LIMIT 5;`
    return new Promise((resolve, reject) => {
      try {
        connection.query(sql, (err: QueryError, result: FeedChildItem[]) => {
          if (err) throw err;
